...
grouped_timepoint = merged_df.groupby(['Timepoint', 'Drug Regimen']).count().reset_index('Drug Regimen')[['Drug Regimen', 'Mouse ID']]	
metastatic_grouped = merged_df.groupby(['Timepoint','Drug Regimen']).mean().reset_index('Drug Regimen')[['Drug Regimen', 'Metastatic Sites']]
...
# -------------------------------------------------------------------------------------- DIV for TITLE
div_title = html.Div(children =	html.H1('Drugs effect on mice'),
					style ={
...
div_2_2_button_1 = dcc.Checklist(
						id = 'checklist-three-categories-survival-function1',
					        options=[
					            {'label': 'lightweight', 'value': 'lightweight'},
					            {'label': 'heavyweight', 'value': 'heavyweight'},
					            {'label': 'placebo', 'value': 'placebo'},
...
div_2_2_button_2 = dcc.Checklist(
							id = 'checklist-three-categories-survival-function2',
					        options=[
...
div_row4 = html.Div(children =	div_4_graph,
...
@app.callback(
    [Output('checklist-three-categories-survival-function2', 'options'),
    Output('checklist-three-categories-survival-function2', 'value')],
    [Input('checklist-three-categories-survival-function1', 'value')])
def set_weights_2_options(selected_categories):
	categories = {'lightweight': ['Capomulin','Ramicane'],
					'placebo': ['Placebo'],
					'heavyweight': ['Ceftamin', 'Infubinol', 'Ketapril', 
...
@app.callback(
    ...
    [Input('checklist-three-categories-survival-function2', 'value')]
)
def update_weight_categories_survival_function(drug_list):

	traces = []
	if drug_list:
	    for drug_name in drug_list:
		    traces.append(go.Scatter(x=grouped_timepoint[grouped_timepoint['Drug Regimen']==drug_name].index,
		    	y = grouped_timepoint[grouped_timepoint['Drug Regimen'] == drug_name]['Mouse ID'],
		    							opacity=0.6,
		    							name = drug_name,
		    							marker = dict(color=drug_colors[drug_name])
					    	)
    ...
    return {
        ...
        'layout': dict(
            ...
            yaxis={'title': 'tumor volume mm3', 
            		'range': [merged_df['Tumor Volume (mm3)'].min(), merged_df['Tumor Volume (mm3)'].max()],
            		'showgrid': False
            		},
            boxmode = 'group',
            autosize=False,
...
def update_metastatic_site(drug_list):
    traces = []
    if drug_list:
	    for drug_name in drug_list:
	    	traces.append(go.Scatter(x=metastatic_grouped[metastatic_grouped['Drug Regimen']==drug_name].index,
		    	y = metastatic_grouped[metastatic_grouped['Drug Regimen'] == drug_name]['Metastatic Sites'],
		    							opacity=0.6,
		    							name = drug_name,
		    							marker = dict(color=drug_colors[drug_name])
					    	)
